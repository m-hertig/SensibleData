...
void PiccoloLib::setup(int xPin, int yPin, int zPin){
    ...
    gotPos = false;
    waitingForPos = false;
    waitingForWheelPos = false;

    drawOrientation = ORIENTATION_BOTTOM;

}

void PiccoloLib::detachServos() {
    X.detachServo();
    Y.detachServo();
    Z.detachServo();
    isServosAttached = false;
}

void PiccoloLib::attachServos() {
    X.attachServo();
    Y.attachServo();
    Z.attachServo();
    isServosAttached = true;
    delay(50);
}
...
void PiccoloAxis::attachServo() {
    servo.attach(pin);
}

void PiccoloAxis::detachServo() {
    servo.detach();
}
...
int PiccoloLib::calcInt(byte b_1, byte b_2, byte b_3, byte b_4){
    int i = (b_1 << 24) | (b_2 << 16) | (b_3 << 8) | (b_4);
    return i;
}
    ...
    if ((Serial.available() > 0)) {
        while (Serial.available() > 0) {
           ...
           if(waitingForPos && inByte == COMMAND_POS_END_BYTE  && index == 12 ) {
            ...
            else if(index==0 && inByte == COMMAND_POS_START_BYTE && !waitingForPos){
                if (!isServosAttached) {
                    attachServos();
                }
                waitingForPos = true;
            ...
            else if(!waitingForPos && !waitingForWheelPos && inByte == COMMAND_CONNECT) {
                index = 0;
                for (int i=0; i<25; i++) {
                    inString[i] = 0;
                }
            ...
            // wheel commands
            else if(waitingForWheelPos && inByte == COMMAND_POS_END_BYTE  && index == 4 ) {

                int newWheelPos;

                // TODO: convert byte directly to int
                newWheelPos = calcInt((byte)inString[3],(byte)inString[2],(byte)inString[1],(byte)inString[0]);
                // newWheelPos = 500;
                index = 0;
                waitingForWheelPos =false;

                wheel.takeSteps(newWheelPos);
                Serial.println(COMMAND_SEND_NEXT);

            }
            else if(index==0 && inByte == COMMAND_ROTATEWHEEL && !waitingForWheelPos) {
                waitingForWheelPos = true;
                index = 0;
            }
                        //Go home
            else if(inByte == COMMAND_END_STACK && !waitingForPos && !waitingForWheelPos) {
                home();
                wheel.idle();
            }
            //everything else
...
//    } else {
//        // Dirty hack to avoid hearing servo noise
//        delay(400);
//        detachServos();
//    }
}

PiccoloWheel::PiccoloWheel(){}

/* ================================= setup functions ================================= */

void PiccoloWheel::setup() {
    stepper.setSpeed(STEPPER_SPEED);
}

void PiccoloWheel::takeSteps(int stepsToTake) {
    stepper.step(stepsToTake);
}

void PiccoloWheel::idle() {
